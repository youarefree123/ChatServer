...
std::pair<int, int> runOnce()
{
  ...
  for (int i = 0; i < numPipes; ++i)
  {
    ...
    channel.set_read_callback(std::bind(readCallback, std::placeholders::_1, channel.fd(), i));
  ...
  int iterTime = static_cast<int>(end.microseconds_since_epoch() - beforeInit.microseconds_since_epoch());
  int loopTime = static_cast<int>(end.microseconds_since_epoch() - beforeLoop.microseconds_since_epoch());
